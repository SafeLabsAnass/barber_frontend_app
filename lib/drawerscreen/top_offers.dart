    ...
    RestClient(RetroApi().dioData()).checkcoupon(code).then((response) {
      setState(() {
        ...
        if (response.success == true) {
          ...
          Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (context)=>ConfirmBooking(widget.selectedEmpId, widget.time, widget.date, totalprice, widget.selectedServices, widget.salonId, widget._selectedServicesName, widget._totalprice, widget.salonData, true, false),),);
        } else {
          ToastMessage.toastMessage(response.msg!);
        }
      });
